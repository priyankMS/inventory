...
import { encrypt } from 'src/modules/auth/guards/bcrypt';
...
export class UserService {
  ...
  async updateUser(userId: string, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    if (updateUserDto.password) {
      userData.password = encrypt(updateUserDto.password);
    }

    const existingUser = await this.userModel.findByIdAndUpdate(userId, userData, {
      new: true,
    }).exec();
